...
def face_module(front_frame_path, side_frame_path, target_path, yolo_model, media_pipe):
    '''
    ...
    side_frame = cv2.imread(side_frame_path)

    hand_dict = inference(side_frame, yolo_model, media_pipe)

    front_frame = cv2.cvtColor(front_frame, cv2.COLOR_BGR2RGB)
    front_frame = cv2.flip(front_frame, 1)
    front_frame = enhance_frame(front_frame)

    target = cv2.imread(target_path)
    #target = cv2.cvtColor(target, cv2.COLOR_BGR2RGB)

    outcome = DeepFace.verify(img1_path=target, img2_path=front_frame, model_name='ArcFace', detector_backend='mediapipe', normalization='ArcFace', align=False, enforce_detection = False)['verified']
    num_faces = numoffaces(front_frame)
    face_dict = face_pose(front_frame)
    #print(hand_dict)
    output = {}
...
if __name__ == '__main__':
    ...
    front_frame_path = 'Images/facecam1.png'
    side_frame_path = 'Images/handcam3.png'
    target_path = 'Images/identity.jpeg'
    result = face_module(front_frame_path, side_frame_path, target_path, yolo_model=model, media_pipe=media_pipe_dict)
    print(result)
