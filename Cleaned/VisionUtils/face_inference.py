...
# Available backends (only 2 for CPU efficiency)
...
#     try:
#         # Validate frames
#         if frame is None or referenceframe is None:
#             return False
        
#         if not hasattr(frame, 'shape') or not hasattr(referenceframe, 'shape'):
#             return False
        
#         if len(frame.shape) != 3 or len(referenceframe.shape) != 3:
#             return False
        
#         # Convert color spaces
#         frame_new = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
#         referenceframe_rgb = cv2.cvtColor(referenceframe, cv2.COLOR_BGR2RGB)
#         cv2.imshow("frame_new", frame_new)
#         cv2.imshow("referenceframe_rgb", referenceframe_rgb)
#         cv2.waitKey(0)
#         cv2.destroyAllWindows()
#         # Call DeepFace verification with enforce_detection=False to handle cases where face detection fails
#         result = DeepFace.verify(
#             img1_path=frame_new, 
#             img2_path=referenceframe_rgb, 
...
#     except Exception as e:
#         # Silent error handling - just return False for failed verification
#         print(f"Error during verification: {e}")
#         return False
